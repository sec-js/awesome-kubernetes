...
23. **YouTube Mosaic Exemption**: The YouTube links comprising the primary visual mosaic (the <center> block with images) in `docs/index.md` and `v2-docs/index.md` are fixed assets. They MUST NOT be included in `data/inventory.yaml` and are exempt from automated health checks. However, all other YouTube resources in the same files (e.g., the collapsible "Top Videos" section or standard links) MUST be checked by the link cleaner.
